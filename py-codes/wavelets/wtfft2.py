...
def fullSky_convolve(healpix_map, radius, primary_beam_fits):
	...
	for j in xrange(4):
		...
		for iter in xrange(0 + j, 16, 4):
			...
			for itr in xrange(npix):							#	npix
		        ...
		        alpha = file('ntrruobjects%d.save'%iter, 'wb')
	...
	for itr4 in xrange(16):		
                alpha = file('ntrruobjects%d.save'%itr4, 'rb')                
...
def run_test():
	...
	for itr in xrange(hdu_gridd[0].data.shape[0]):
		...
		tru_image_data = fullSky_convolve(stokes_p,radd/60. ,'mueller_4_x_4_beam.fits')
		np.save('nchan_Trrmap'+ str(itr), tru_image_data) 
		trumap.append(tru_image_data)
	pf.writeto('conv_map_560_x_16_786432_trubeam.fits', np.array(trumap), clobber=True)
	#radd = 150.
