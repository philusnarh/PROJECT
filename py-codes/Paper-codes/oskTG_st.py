...
#
def f_healpix_plott(conv_map, figname, fig_num, w_size, h_size, DPI, cmap=None):
		...
		if i in xrange(1):
			hp.visufunc.mollview(abs(conv_map[i,...]), coord = ['G', 'C'], 
					    norm= 'log', xsize = 3200, cbar=True,
			                    ...
			                    notext = True, sub = (1,3 , i + 1), cmap = cmap)		
		else:
			hp.visufunc.mollview(conv_map[i,...], coord = ['G', 'C'], 
			 		     norm= 'linear', xsize = 3200, cbar=True, 
					     unit = 'Jy/pixel',  margins=(0.01, 0.01, 0.0, 0.02),
					     notext = True, sub = (1, 3, i + 1), cmap = cmap) 
...
#
def conv_healpix_plott_all(conv_map, figname, fig_num, w_size, h_size, DPI, cmap = None):
	fig = plt.figure(fig_num, figsize=(w_size, h_size), dpi= DPI)
	...
	for i in xrange(conv_map.shape[0]):	
		...
		if i in xrange(6):
			hp.visufunc.mollview(abs(conv_map[i,...]), coord = ['G', 'C'], 
			                     ...
			                     notext = True, sub = (3,3 , i + 1), cmap = cmap)
			                     #		
		else:
...
#
def conv_healpix_plott(conv_map, figname, fig_num, w_size, h_size, DPI, cmap=None):
	fig = plt.figure(fig_num, figsize=(w_size, h_size), dpi= DPI)
	k = ['I', 'Q', 'U', 'V']*4
	k1 =  sorted(k)
	j = 0
	for i in xrange(conv_map[0].shape[0]):	         # [0, 1, 2, 4, 5, 6, 8, 9, 10]:
		if i in [0, 4, 8]:   		#[0]
			...
			j = j + 1
			hp.visufunc.mollview(abs(conv_map[0, i,...]), coord = ['G', 'C'], 
					     norm= 'log', xsize = 3200, cbar=True, 
					     unit = 'Jy/beam', margins=(0.01, 0.01, 0.0, 0.02), 
					     notext = True, sub = (3,3 , j), cmap = cmap)
			...
			j = j + 1
			hp.visufunc.mollview(conv_map[0, i,...], coord = ['G', 'C'], 
					     norm= 'linear', xsize = 3200, cbar=True, 
					     unit = 'Jy/beam', margins=(0.01, 0.01, 0.0, 0.02), 
					     notext = True, sub = (3, 3, j ), cmap = cmap) 
			plt.title(r'$ %s \longrightarrow %s $' %(k[i], k1[i]), fontsize=20, fontweight="bold")
...
#
def error_conv_healpix_plott(conv_map, figname, fig_num, w_size, h_size, DPI, cmap = None):
	fig = plt.figure(fig_num, figsize=(w_size, h_size), dpi= DPI)
	k = ['I', 'Q', 'U']*3
	k1 =  sorted(k)
	j = 0
...
def run_test():
	#       start-time
        ...
        option.add_option('-f','--file1', dest='setup',
                          default='newsetup_GP.ini', action="store", type="string",
	...
	for iter in [chan_map]: #xrange(1):
		...
		conv_healpix_plott(conv_map = v, figname = '%s_Tchan_%d.png' %(strt_sav_file, iter), fig_num = 1 ,
		                   w_size = 18, h_size = 10, DPI = 500, cmap = get_cmap)
		...
		f_healpix_plott(conv_map = dat[iter], figname = '%s_fchan_%d' %(strt_sav_file, iter), fig_num = 3 , 
		                   w_size = 14, h_size = 3, DPI = 500, cmap = get_cmap)
		...
		error_conv_healpix_plott(conv_map = np.array(conv_errors), figname = '%s_Echan_%d.png' %(strt_sav_file, iter),
		 			fig_num = 15, w_size = 18, h_size = 10, DPI = 500, cmap = get_cmap)
		print '\n >>> done !!!!!!!!!!'
...
#		lst.append(tu - du)
		...
		lst.append(du - tu)
		#
#		lst.append(abs(di) - abs(ti))
#		lst.append(abs(dq)- abs(tq))
#		lst.append(abs(du)- abs(tu))
		...
		ax1.semilogy(lmax,lmax*(lmax +1.)*ud/2.0*np.pi, label = 'Conv.$I$ with ${MB_{GP}}$ ')
		...
		ax2.semilogy(lmax,ud/u1, 'darkviolet', label = '${MB_{GP}}$ ') 		
		...
		ax1.semilogy(lmax,lmax*(lmax +1.)*ud/2.0*np.pi, label = 'Conv.$Q$ with ${MB_{GP}}$ ')
		...
		ax1.semilogy(lmax,lmax*(lmax +1.)*ud/2.0*np.pi, label = 'Conv.$U$ with ${MB_{GP}}$ ')
