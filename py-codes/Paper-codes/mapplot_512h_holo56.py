...
def f_healpix_plott(conv_map, figname, fig_num, w_size, h_size, DPI):
	...
	for i in xrange(conv_map.shape[0] - 1):	
		...
		if i in xrange(1):
			...
			unit = 'Jy/pixel', margins=(0.01, 0.01, 0.0, 0.02), notext = True, sub = (1,3 , i + 1))		
		else:
...
def conv_healpix_plott_all(conv_map, figname, fig_num, w_size, h_size, DPI):
	fig = plt.figure(fig_num, figsize=(w_size, h_size), dpi= DPI)
	k = ['$Measured \, Stokes \, I^{HB_{ant5}}$', '$Measured \, Stokes \, Q^{HB_{ant5}}$', 
	     '$Measured \, Stokes \, U^{HB_{ant5}}$', '$Measured \, Stokes \, I^{HB_{ant6}}$',
	      '$Measured \, Stokes \, Q^{HB_{ant6}}$', '$Measured \, Stokes \, U^{HB_{ant6}}$', 
	     '$ Error \, in \, Stokes \, I$', '$ Error \, in \, Stokes \, Q$', '$ Error \, in \, Stokes \, U$' ]
	#k1 =  sorted(k)
	for i in xrange(conv_map.shape[0]):	
		#print conv_map.shape[0]
		if i in xrange(6):
			hp.visufunc.mollview(abs(conv_map[i,...]), coord = ['G', 'C'], norm = 'log', xsize = 3200, cbar =True,\
			unit = 'Jy/beam', margins = (0.01, 0.01, 0.0, 0.02), notext = True, sub = (3,3 , i + 1))		
		else:
			hp.visufunc.mollview(conv_map[i,...], coord = ['G', 'C'], norm= 'linear', xsize = 3200, cbar = True,\
			unit = 'Jy/beam', margins = (0.01, 0.01, 0.0, 0.02), notext = True, sub = (3, 3, i + 1)) 
		plt.title(r'%s'  % k[i], fontsize = 20, fontweight="bold") 
...
def run_test():
	#       start-time
        ...
        option.add_option('-f','--file1', dest='setup',
                          default='newsetup_hh.ini', action="store", type="string",
...
#		                   
		...
		for jj in xrange(16):
			...
			if jj in [3, 7, 11, 12, 13, 14, 15]:
				...
				#conv_errors.append(abs(v1[0, jj, ...]) - abs(v[0, jj, ...]))
		...
		np.save('h56_error_%d' %iter, np.array(conv_errors)) 
...
#		
		...
		m_IT = v[0, 0, ...] + v[0, 1, ...] + v[0, 2, ...]
		m_ID = v1[0, 0, ...] + v1[0, 1, ...] + v1[0, 2, ...]
		#m_IT = v[0, 0, ...] + savgol_filter(v[0, 1, ...] + v[0, 2, ...], 21, 3, deriv = 2) 
		#m_ID = v1[0, 0, ...] + savgol_filter(v1[0, 1, ...] + v1[0, 2, ...], 21, 3, deriv = 2)
		...
		ax1.semilogy(lmax,lmax*(lmax +1.)*u/2.0*np.pi, label = 'Conv. $I$ with $HB_{ant5}$ ')		 
		ax1.semilogy(lmax,lmax*(lmax +1.)*ud/2.0*np.pi, label = 'Conv.$I$ with $HB_{ant6}$ ')
		...
		ax2.semilogy(lmax,u/u1, '-y', label = 'Ant5 Holo Beam ')
		ax2.semilogy(lmax,ud/u1, 'darkviolet', label = 'Ant6 Holo Beam ') 		
		...
		m_QT =	v[0, 5, ...] + savgol_filter(v[0, 6, ...], 91, 5, deriv = 2) 		
		m_QD =	v1[0, 5, ...] + savgol_filter(v1[0, 6, ...], 91, 5, deriv = 2) 
		...
		ax1.semilogy(lmax,lmax*(lmax +1.)*u/2.0*np.pi, label = 'Conv. $Q$ with $HB_{ant5}$ ')		 
		ax1.semilogy(lmax,lmax*(lmax +1.)*ud/2.0*np.pi, label = 'Conv.$Q$ with $HB_{ant6}$ ')
		...
		m_UT =	v[0, 10, ...] + savgol_filter(v[0, 9, ...], 91, 5, deriv = 2) 		
		m_UD =	v1[0, 10, ...] + savgol_filter(v1[0, 9, ...], 91, 5, deriv =2) 
		...
		ax1.semilogy(lmax,lmax*(lmax +1.)*u/2.0*np.pi, label = 'Conv. $U$ with $HB_{ant5}$ ')		 
		ax1.semilogy(lmax,lmax*(lmax +1.)*ud/2.0*np.pi, label = 'Conv.$U$ with $HB_{ant6}$ ')
