#
"""
	 This script generates 2x2 Jones & 4x4 Mueller Beams Plots from OSKAR Beampattern simulations. It also 
...
def jones_filename(beam_dir1, beam1_start_filename, station_num, chan_num):
	...
	for ii in xrange(len(ty)):
		...
		for jj in xrange(len(jns)):			
			beam_file = '%s/%s_jones_S%s_%s_%s.fits' %(beam_dir1, beam1_start_filename, str(station_num).zfill(4), jns[jj], ty[ii]) 
	...
	if os.path.exists('%s' %options.configFile):
		...
		while jj < total_num_stations:
			...
			fv1 = figur(1, 12, 12, 70)
	    		Jones_image(fv1, beam_header, 2, 2, 4 ,  beam_data_amp, '%s_S%s_2x2_Jones1_Real_Image.png' %(beam_image1, str(station_num)))		    	
	    		fv2 = figur(2, 12, 12, 70)
	    		Jones_image(fv2, beam_header, 2, 2, 4 , beam_data_ph, '%s_S%s_2x2_Jones1_Imag_Image.png' %(beam_image1, str(station_num)))
			...
			if save_fits == 'yes':
				...
				pyfits.writeto('%s_S%s_mueller_4_x_4_beam1.fits' %(beam_image1, str(station_num).zfill(4)), mueller_beam1, header = beam_header, clobber=True)
			
		    	Mueller_image(fv1, beam_header, 4, 4, 16 , mueller_beam1, '%s_S%s_Meuller_4_X_4_Images1.png' %(beam_image1, str(station_num).zfill(4)))
				...
				#	Plot Jones terms
				fv1 = figur(1, 12, 12, 70)
		    		Jones_image(fv1, beam_header, 2, 2, 4 ,  beam_data_amp, '%s_S%s_2x2_Jones2_Real_Images.png' %(beam_image2, str(station_num).zfill(4)))		    	
		    		fv2 = figur(2, 12, 12, 70)
		    		Jones_image(fv2, beam_header, 2, 2, 4 , beam_data_ph, '%s_S%s_2x2_Jones2_Imag_Images2.png' %(beam_image2, str(station_num).zfill(4)))
				...
				if save_fits == 'yes':
					...
					pyfits.writeto('%s_S%s_mueller_4_x_4_beam2.fits' %(beam2_start_filename, str(station_num).zfill(4)), mueller_beam2, header = beam_header, clobber=True)
			    	Mueller_image(fv1, beam_header, 4, 4, 16 , mueller_beam2, '%s_S%s_4x4_Meuller_Images.png' %(beam_image2, str(station_num).zfill(4)))
			    	...
			    	if save_fits == 'yes':
			    		...
			    		pyfits.writeto('%s_S%s_mueller_4_x_4_beam.fits' %(beam_diffname, str(station_num).zfill(4)), diff_beam, header = beam_header, clobber=True)
			    	fv1 = figur(4, 12, 12, 80)
			    	Mueller_image(fv1, beam_header, 4, 4, 16 , diff_beam, '%s_S%s_4x4_Meuller_Images.png' %(beam_image2, str(station_num).zfill(4)))
