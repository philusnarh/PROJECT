...
h5 = katdal.open('%s' %h5file)     		# +++++++++++++
h5.file.close()

#corr_products = h5.corrprods
corr_products = h5.corr_products
...
msfil = 'k7MS'
msloc = '/home/theonarh/Documents/mario/osk_sim/mkk/%s' %msfil
...
#     #for val in corr_index1:
    auto1 = corr_products.tolist().index(['ant%dh' %val,'ant%dh' %val])
    #auto1 = h5.lookup['ant%dh' %val,'ant%dh' %val]    
    vis_data[:ss[0],:ss[1],auto1,0] = vish
    #auto2 = h5.lookup['ant%dv' %val,'ant%dv' %val]
    auto2 = corr_products.tolist().index(['ant%dv' %val,'ant%dv' %val])
