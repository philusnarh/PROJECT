...
def run_test():
	...
	if os.path.exists('%s' %options.filename1):		
		...
		if len(shape) == 3:		
			...
			#				
			for itr in [10, 382]: #xrange(shape[0]):	
			...
			for iter in [10, 382]:	#xrange(shape[0]):					
				alpha = file('healpix_map_%d.save' %iter, 'rb')				             
				loaded_objects.append(cPickle.load(alpha))				
			alpha.close()
			measured_map.append(copy.deepcopy(loaded_objects)) 			
			pf.writeto('%s_with_NSIDE_%d_NPIX_%d.fits' %(start_save_as_name, Nside, Npix), np.array(measured_map), clobber=True)
			#
