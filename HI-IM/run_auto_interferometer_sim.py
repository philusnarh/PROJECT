...
def run_sim_test():
	...
	if os.path.exists('%s' %options.setup1):
		...
		if config.get('simulations', 'run_interferometer_sim', 1) == 'false':
			print "\n >> Error: config file for input parameters does not exist"       		
       			raise
		
		else:	
			if os.path.exists('%s' %options.setup2) == False:
				print "\n >> Error: OSKAR setup file does not exist"       		
	       			raise	
			
			else:	       			
				msname = config.get('simulations', 'run_interferometer_sim\msname', 1)
				msdir = config.get('simulations', 'run_interferometer_sim\ms_dir', 1) 
				...
				os.system('oskar_settings_set  %s observation/num_channels %d' %(options.setup2, 1))
				#
				if os.path.isdir("%s" %msdir):		
	       				os.popen('rm -rf %s' %msdir)
				#os.makedirs('%s' %filename)
				os.mkdir("%s" %msdir)
				for iter in xrange(nchanns):
					...
					os.system('oskar_vis_to_ms %s -o ./%s/%s_chan%s.MS' %(visname, msdir, msname, str(iter).zfill(4)))
					#msname = msname[:-3].strip().replace(" ","")
					os.system('cp %s  ./%s/%s_chan%s.vis' %(visname, msdir, msname, str(iter).zfill(4)))
					start_freq += freq_inc
					
				os.system('oskar_vis_add ./%s/*.vis -o combined.vis' %(msdir))
				os.system('oskar_vis_to_ms ./%s/combined.vis -o complete.ms' %(msdir))
				#os.system('pyxis  *MS per_ms[ms.concat]')
				#os.system('rm -rf *.log ./%s/*chan*.vis ' %(msdir))				
				os.system('oskar_settings_set  %s observation/start_frequency_hz %f' %(options.setup2, sf))
