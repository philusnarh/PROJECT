...
This program models an antenna dish as a collection of dipoles 
...
def feed_angle_displacement(input_directory, sub_directory, uniform_error, alpha_x, alpha_y, n_antenna):
	...
	if uniform_error == 'false':
		...
		for iter in xrange(n_antenna):	
			feed_x =  np.repeat(alpha_x, shape[0]) 	+ np.random.uniform(alpha_x, alpha_y, 1)	 			
...
def add_gain_phase_error(input_directory, sub_directory, uniform_error, gain_f, phase_off, time_gain_std, time_phase_std, n_antenna):
	...
	if uniform_error == 'false':
		...
		for iter in xrange(n_antenna):	
			G_0 =   np.repeat(a = gain_f, repeats = shape[0], axis=None)		
			phi_deg = np.repeat(a = phase_off, repeats = shape[0], axis=None)			
			G_std = np.random.normal(loc=0.0, scale = time_gain_std, size = shape[0])			
			phi_std = np.random.normal(loc=0.0, scale = time_phase_std, size = shape[0])
...
def run_dish_sim_model():
	...
	option.add_option('-f','--file1', dest='config', 
	                  ...
	                  help='Enter the input parameters in sections 1 & 2 of beamsetup.ini')
	...
	if os.path.exists('%s' %options.config):		
		...
		plt.hist(dish_aperture/2.0*radial_distribution,100, alpha = 0.9, weights = np.zeros_like(radial_distribution)\
		+ 1./ radial_distribution.size)
		plt.xlim(0, dish_aperture/2.0)
		plt.xlabel('Radius / m') 
		plt.ylabel('Distribution of Aperture Illumination')
#		plt.title('Super-Gaussian Distribution')
		...
		#	Apply Dish support
			...
			r1 = beam_supt #/4.0
			a = radial_distribution[np.where((radial_distribution<=1) & (radial_distribution> 0.2))]
			U = np.random.uniform(0, 2*np.pi, len(a))	   
...
#			uu2 = 750
			print len(s1[0])
			print len(s1[0])/uu
			s1 = np.take(s1, range(len(s1[0])- len(s1[0])/uu))
#			s1 = np.take(s1, range(uu))
			x1 = np.delete(x, s1)
			y1 = np.delete(y, s1)
			s2 = np.where(np.logical_and(y1 >= - r1, y1 <= r1))
			print len(s2[0])
			print len(s2[0])/uu
			s2 = np.take(s2, range(len(s2[0]) - len(s2[0])/uu))
#			s2 = np.take(s2,  range(uu))
			x1 = np.delete(x1, s2)
			y1 = np.delete(y1, s2)		    
		#
	...
	if config.get('distort_antenna-layout', 'include_distortion\per_station', 1) == 'true':
		...
		if config.get('distort_antenna-layout', 'add_gain_phase_error', 1) == 'true':
			...
			add_gain_phase_error(input_directory = ant_directory, sub_directory = ant_sub_directory, uniform_error = err_type, gain_f = xx_0,
		...
		if config.get('distort_antenna-layout', 'introduce_feed_displacement', 1) == 'true':
			...
			feed_angle_displacement(input_directory = ant_directory, sub_directory = ant_sub_directory, uniform_error = err_type,
					        alpha_x = f_x, alpha_y = f_y, n_antenna = num_ant)
